...
<div class="min-h-[100dvh] bg-slate-900 text-slate-100 flex flex-col select-none">
  ...
  <main class="flex-1 flex flex-col px-4 py-5 gap-4 max-w-lg mx-auto w-full">
    ...
    {#if q.drillType === 'nextChord'}
      <NoteWheel suffix={q.suffix} onPick={pick} selected={selected} correct={q.correct} />
    ...
    {:else}
      <div class="grid grid-cols-2 gap-3">
        {#each q.options as opt}
          ...
          <div class="py-5 px-3 rounded-xl font-bold text-xl text-center transition-all duration-200
                      {isCorrect
                        ? 'bg-emerald-500 text-white ring-2 ring-emerald-300 scale-105'
                        : isWrong ? 'bg-red-500 text-white' : 'bg-slate-800 text-slate-600'}"
          >{opt}</div>
        {/each}
      </div>
    {/if}
